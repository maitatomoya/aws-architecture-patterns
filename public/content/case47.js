...
registerCase({
  ...
  requirements: [
    ...
    "専任者なしで回る運用負荷にしたい",
    "追加のサーバーを立てたくない",
    "設定ミス（公開設定の誤りなど）の検知は次の段階とし、まずは脅威検知と監査証跡を先に整えたい"
  ],
  main: {
  ...
  quiz: [
    {
      q: "GuardDutyを入れれば安心とは言えません。このサービスが担当するのはどこまでで、その先には何が必要でしょうか。",
      a: "GuardDutyが担うのは検知までで、通信の遮断や設定の修復まではしてくれません。検知結果をEventBridgeで振り分け、SNSで通知し、人が対応する（あるいはLambdaで自動対処を作る）ところまで含めて初めて要件を満たします。検知と対応は別の仕事だと分けて考えるのが、セキュリティ運用を設計するときの基本です。"
    },
    {
      q: "「S3バケットが公開設定になっている」といった設定ミスは、この構成では気づけません。それはなぜで、どのサービスの担当でしょうか。",
      a: "GuardDutyは操作ログや通信ログから普段と違う怪しい動きを見つける脅威検知のサービスで、リソースの設定が基準から外れていないかを点検する役割は持たないためです。設定ミスの継続チェックはAWS ConfigやSecurity Hub（CSPM）の担当で、この構成に次の一歩として足すのが定番です。脅威検知と設定監査は別の領域だと分けて覚えておくと、要件の抜け漏れに気づけます。"
    },
    {
      q: "検知した内容をすべてチャットに流したところ、1週間で誰も見なくなりました。あなたならどう立て直しますか。",
      a: "EventBridgeのルールで重要度が高いものに絞り、通知は鳴ったら必ず誰かが動くものだけに限定します。全件通知は狼少年化して形骸化を招き、結果として重大な検知まで埋もれてしまうためです。残りの低重要度の検知はダッシュボードや定期レビューで消化する、と経路を分けます。ケース46のアラート疲れと同じ構図で、通知の設計そのものが運用設計の一部だと考えましょう。"
    }
  ]
});
