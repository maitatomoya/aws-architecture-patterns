...
registerCase({
  ...
  quiz: [
    {
      q: "メトリクス・ログ・トレースの3つが揃って初めて監視が成立すると言われるのはなぜでしょうか。役割の違いから説明してみましょう。",
      a: "3つは役割が違い、互いに代わりが利かないためです。メトリクスは数値の傾向から異常が起きたことに気づくため、トレースは複数サービスをまたぐ1リクエストを追ってどこが遅いかを絞るため、ログは絞り込んだ箇所で何が起きたかを読むためのものです。どれかが欠けると、気づけない・場所を特定できない・原因が読めない、のいずれかで調査が止まります。"
    },
    {
      q: "アラームを最初から増やしすぎないほうがよいのはなぜでしょうか。",
      a: "通知が多すぎると人が反応しなくなり、本当に重要な通知まで見過ごされるからです（アラート疲れ）。まずはエラー率・レイテンシ・死活といったユーザー影響に直結する少数の指標から始め、鳴ったときの対応が決まっているものだけを通知に載せます。アラームの本数を増やすことよりも、鳴ったら必ず誰かが動く状態を保つほうが優先されます。"
    },
    {
      q: "「ユーザーIDを指定して過去1か月のログを横断検索したい」という調査依頼が毎週来るようになったら、あなたなら構成をどう育てますか。",
      a: "検知と通知はCloudWatchのまま残し、ログ検索だけOpenSearch（代替1）を足す併用構成にします。全文検索と柔軟な集計はOpenSearchの得意分野ですが、クラスターのサイズ設計やインデックス管理という新しい運用が発生するため、検索対象は直近数週間に絞り、全期間の生ログはS3へ置くのが費用設計の定石です。その前に、ログをJSONで構造化してLogs Insightsで足りないかを確かめる順序も現実的です。"
    }
  ]
});
