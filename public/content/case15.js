...
registerCase({
  ...
  quiz: [
    {
      q: "SQSを使うとき、デッドレターキュー（DLQ）を最初から用意しておくべきなのはなぜでしょうか。",
      a: "DLQがないと、処理に失敗したメッセージが延々と再試行され続け、ワーカーの実行料金を食いながら他のジョブの処理も圧迫します。数回失敗したらDLQへ退避する設定にしておけば、失敗に気づけて、原因を直したうえで再投入できます。失敗するジョブは必ず出るという前提で、退避先を後付けではなく最初から設計に入れておくのが実務です。"
    },
    {
      q: "SQSでは同じメッセージが稀に二回配信されることがあります。この仕様に対して、ワーカー側はどう作っておくべきでしょうか。",
      a: "同じジョブを二回実行しても結果が変わらない冪等な作りにしておきます。処理済みのジョブIDをDynamoDBに記録し、すでに記録があればスキップするのが典型的な実装です。可視性タイムアウトをジョブの最大処理時間より長く設定して二重配信自体を減らす対策と合わせ、両面から備えるのが安全です。"
    },
    {
      q: "「機械学習の前処理を一件あたり四十分かけて回したい」という依頼が来ました。あなたならこの構成をどう変えますか。",
      a: "Lambdaは実行時間が最大十五分なので、この処理はそのままでは載せられません。実行部分をAWS Batchへ差し替えれば実行時間やメモリの制限が実質なくなり、スポットインスタンスを併用して重い計算のコストも抑えられます。SQS+Lambdaと排他ではないので、軽いジョブはLambda、重いジョブはBatchと振り分ける併用が実務では一般的です。"
    }
  ]
});
