...
registerCase({
  ...
  quiz: [
    {
      q: "推奨構成では動画をHLS（数秒単位に分割された配信形式）へ変換してから配信しています。元のMP4をそのままCloudFrontで配信するのと比べて何が良いのでしょうか。",
      a: "HLSは回線速度に応じて画質を切り替えられるため、電波の悪い場所のスマホでも再生が止まりにくくなります。さらに細切れのファイルになることでCDNのキャッシュが効きやすく、視聴者が一万人いてもオリジンのS3から取得されるのは各ファイル一回で済み得ます。視聴体験と配信コストの両方に効く変換だと理解しておきましょう。"
    },
    {
      q: "元動画を置くバケットと配信用バケットをわざわざ分けているのはなぜでしょうか。",
      a: "元動画は誰にも公開したくないのに対し、変換後のファイルはCloudFront経由で広く配信されるため、必要な権限設定が正反対になるからです。バケットを分ければ「元動画は非公開のまま」という設定を単純に保てますし、元動画だけライフサイクルルールで低頻度アクセス層へ移して保存費を下げる管理もしやすくなります。"
    },
    {
      q: "「この講座は有料会員だけに見せたい」という要件が追加されました。あなたなら構成をどう変えますか。",
      a: "配信基盤であるS3+CloudFrontはそのままに、CloudFrontの署名付きURLまたは署名付きCookieによるアクセス制御を重ねます。会員かどうかを判定して署名を発行する小さなAPIを用意し、S3側はOACでCloudFront経由以外を全拒否したままにするのが肝です。画面録画まで防ぎたいという要求ならDRMの領域になるため、どこまでの強度が必要かを先に確認します。"
    }
  ]
});
