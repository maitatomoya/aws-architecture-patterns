...
registerCase({
  ...
  quiz: [
    {
      q: "認証チェックをLambdaの中ではなく、API Gatewayのオーソライザーで行うのはなぜでしょうか。",
      a: "不正なリクエストをLambdaが起動する前に弾けるため、攻撃を受けても実行料金が発生しません。加えて三十本のエンドポイントそれぞれに認証コードを書かずに済むので、一本だけ実装を忘れて認証がすり抜けるという事故を構造的に防げます。共通の関心事は入口に集約する、という設計の基本がそのまま現れた形です。"
    },
    {
      q: "この構成にはVPCもインターネットゲートウェイも登場しません。図から省略しているだけなのでしょうか。",
      a: "省略ではありません。API Gateway・Lambda・DynamoDBはすべてVPCの外で動くマネージドサービスで、利用者はAWSが運用する公開エンドポイントを使うためです。ネットワーク設計をせずにHTTPSのAPIを公開できること自体がサーバーレスの価値で、VPCが必要になるのはRDSのように自分のネットワーク内へ置くリソースを使うときです。"
    },
    {
      q: "「PDFを生成する処理に一分かかる」という要件が追加されました。あなたなら構成をどう変えますか。",
      a: "API Gatewayの二十九秒制限に引っかかるため、同期でレスポンスを返す設計は成立しません。ケース15で学ぶようにSQSへ積んで即座に受付応答を返し、生成が終わったら結果を取りに来てもらう非同期化がまず有力です。どうしても同期で返したい事情があるなら、ALB+ECS Fargateへ移して実行時間制限のない常駐APIにする選択になります。"
    }
  ]
});
