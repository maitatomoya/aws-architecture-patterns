...
registerCase({
  ...
  quiz: [
    {
      q: "信頼度スコアで「自動非公開」と「人が確認」に分けています。すべて自動非公開にしてしまうと何が起きるでしょうか。",
      a: "判定は確率的で誤りがゼロにならないため、正常な商品が誤って非公開にされ、出品者からの問い合わせや信頼低下につながります。逆にすべて人が見る運用では工数が減りません。スコア90以上は自動、60から90は通知して人が確認、と分ければ、明らかな違反には即対処しつつ、判断が微妙なものにだけ人の目を使えます。AIは疑いを絞る道具で、最終判断は人が持つという設計です。"
    },
    {
      q: "Rekognitionは1枚あたりの単価が読みやすいのに対し、Bedrock案は費用が読みにくくなります。これはなぜでしょうか。",
      a: "Rekognitionは画像1枚いくらの課金ですが、Bedrockは入力と出力のトークン量に比例するため、画像サイズや返させる説明文の長さで1枚あたりの費用が変わるからです。判定基準を細かく書いたプロンプトや長い理由説明を毎回生成させると単価が跳ねます。まず軽量モデルで実測し、精度が足りない画像だけ上位モデルへ回す設計にすると読みやすくなります。"
    },
    {
      q: "自社規約のNGカテゴリが頻繁に見直され、運営チームには違反理由の説明文も見せたいと言われました。あなたならどの構成を選ぶでしょうか。",
      a: "代替2のBedrockマルチモーダル案へ寄せます。判定基準を自然言語のプロンプトで書けるため、規約が変わってもモデルの再学習ではなく指示文の書き換えだけで即日反映でき、違反理由の説明文も同時に生成できるからです。基準が独自でも安定しているなら、学習データを貯めてSageMakerのカスタムモデルという分岐もあります。いずれにせよ出力はJSON形式に固定し、解析に失敗したときの再試行を必ず入れます。"
    }
  ]
});
