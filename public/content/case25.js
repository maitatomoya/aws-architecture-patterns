...
registerCase({
  ...
  quiz: [
    {
      q: "商品データの正本をDynamoDBに置いたまま、検索だけをOpenSearchに任せています。この役割分担の利点と、代わりに引き受けることになる面倒は何でしょうか。",
      a: "検索インデックスを「正本から作り直せる派生データ」と割り切れるため、壊れたら再構築すればよく、障害復旧の考え方が単純になります。検索負荷が正本DBに一切かからないのも利点です。一方で二重管理になるため、Streams同期に数秒の遅延がある前提で、在庫の最終確認など厳密さが要る処理はDynamoDBへ直接問い合わせる、という使い分けが必要になります。"
    },
    {
      q: "検索がほとんど使われない月でもOpenSearchの費用が下がらないのはなぜでしょうか。",
      a: "OpenSearchはデータノードを常時起動して検索インデックスを保持する仕組みで、費用は検索回数ではなくノードの稼働時間で決まるからです。そのため検索がゼロの月でも月1.5万円程度からの固定費が発生します。検索がサービスの主要機能になるまでは固定費のない代替案で始め、検索体験が売上に効く段階になってから導入する、という判断も十分あり得ます。"
    },
    {
      q: "要件を確認したところ、必要な検索は「カテゴリで絞る」「新着順に並べる」だけで、あいまい検索は不要と分かりました。あなたならどう作るでしょうか。",
      a: "代替2のDynamoDBのGSI設計で作ります。追加のミドルウェアも固定費もなく、ミリ秒で返り、セール時のアクセス急増にも自動で追従するためです。ただし部分一致・表記ゆれ・関連度順は原理的にできないので、その線引きを企画側と最初に合意しておくことが肝心です。検索パターンが3つ4つを超えて増え続けるようなら、検索エンジン導入を検討するサインになります。"
    }
  ]
});
