...
registerCase({
  ...
  quiz: [
    {
      q: "推奨構成では、API GatewayがLambdaを介さずSQSへ直接メッセージを投入しています。間にLambdaを挟まないのはなぜでしょうか。",
      a: "Lambdaを挟むと同時実行数の上限や初回起動の遅延（コールドスタート）の影響を受け、外部SaaSが求める数秒以内の応答を返せない場合があるためです。SQSへ直接書き込めば受付は「メッセージを預かる」だけで完結し、後段の処理の重さや障害と切り離して即座に200を返せます。この構成が狙う<strong>受信と処理の分離</strong>が、最初の1本の矢印の時点で実現されているわけです。"
    },
    {
      q: "同じWebhookが2回届いたとき、SQSやLambdaの仕組みが自動で二重処理を防いでくれるでしょうか。",
      a: "防いでくれません。Webhookも標準キューのSQSも「少なくとも1回は届ける」という前提で作られており、重複配信は障害ではなく正常動作の範囲です。イベントIDをDynamoDBへ条件付き書き込みし、既にあればスキップするという冪等化を自分で実装する必要があります。仕組みが守ってくれる範囲と自分で書く範囲の線引きが、この構成でつまずきやすい点です。"
    },
    {
      q: "取引のあるSaaSが1社に絞られ、それがEventBridgeのパートナー対応サービスだと分かりました。あなたなら構成をどう変えるでしょうか。",
      a: "代替1のEventBridgeパートナーイベント連携へ寄せます。SaaS側とAWS側で連携を承認するだけで安全な経路ができ、API Gateway・エンドポイント公開・署名検証の実装と運用が丸ごと不要になるためです。ただしパートナー未対応の送信元が1社でも残るなら、その分だけ推奨構成を併用するハイブリッドにします。イベント内容で宛先を振り分ける考え方はケース19のイベント駆動設計と同じものです。"
    }
  ]
});
