...
registerCase({
  ...
  quiz: [
    {
      q: "利用者は数百名で性能の心配はほとんどないのに、あえてALBとECSを使っています。なぜ一台のサーバーで済ませないのでしょうか。",
      a: "コンテナを一台ずつ入れ替えるローリング更新ができるため、平日日中の利用時間帯でもサービスを止めずにリリースできるからです。取引先が業務で使うポータルは「メンテナンスのため停止します」という告知自体がコストになります。性能ではなく可用性と運用のしやすさのために構成を選ぶ場面がある、という判断軸を押さえておきましょう。"
    },
    {
      q: "サーバーレス案の図にはVPCが登場しません。「VPCがないぶん守りが弱い」と言えるでしょうか。",
      a: "言えません。守る場所がネットワークの境界からIAMの権限設定とトークン検証へ移るだけで、各サービスへのアクセスはすべて認証・認可を通ります。入口にWAFとCognitoオーソライザーを置けば、推奨構成と同水準の入口防御を保てます。ただし取引先への説明では構成図で示せること自体が価値になるため、何をもって安全と説明するかは事前に整理しておきます。"
    },
    {
      q: "取引先が大企業ばかりで「インターネット経由のアクセスは認められない」と言われました。あなたならどう提案しますか。",
      a: "まず要求の強さを確認します。本当に閉域が必須ならSite-to-Site VPNでインターネットに公開しない構成になりますが、取引先ごとに機器設定が必要で、数十社になると運用が破綻します。取引先の固定IPだけをWAFのIPセットで許可する方式なら相手側の作業が不要で多数にも対応できるため、必要な強度を聞いたうえで選択肢を並べて提案します。"
    }
  ]
});
