...
registerCase({
  ...
  quiz: [
    {
      q: "ライブ配信の自前構築は難度が高いと言われます。IVSは具体的に何を肩代わりしてくれているのか、自分の言葉で説明してみましょう。",
      a: "ライブは映像の受信、配信形式への変換、同時視聴者数に応じた大規模配信、低遅延の維持を同時に満たす必要があり、特に視聴者数の増減に合わせた配信サーバーの増減設計が最大の難所です。IVSはこの一式をサービスとして引き受けるため、利用者は送出先のエンドポイントと再生プレイヤーだけを扱えばよくなります。少人数チームでも本番品質のライブが出せるのは、この難所を丸ごと任せているからです。"
    },
    {
      q: "チャットの接続情報（どの接続がどのルームにいるか）を、Lambdaの中ではなくDynamoDBに持たせているのはなぜでしょうか。",
      a: "Lambdaは呼び出しごとに別のインスタンスで動く可能性があり、前回の実行の記憶は残りません。接続情報をDynamoDBという外部の共有場所に置くことで、どのLambdaが処理しても同じ状態を見られるようになります。状態を外に出して処理をステートレスにするのは、WebSocket構成に限らずスケールする仕組みに共通する原則です。"
    },
    {
      q: "企画が「双方向のオンライン面談」に変わり、遅延を一秒未満に抑えたいと言われました。あなたならどう判断しますか。",
      a: "数秒の遅延を前提とした通常のIVS配信では要件を満たせないため、IVSのリアルタイム機能やWebRTC系の別設計へ切り替える必要があります。許容できる遅延は構成そのものを変える要件なので、企画段階で「何秒までなら成立するか」を必ず確認します。逆に見逃し配信を足したいだけなら、録画をケース4のVODパイプラインへ流す拡張で済みます。"
    }
  ]
});
