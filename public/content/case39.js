...
registerCase({
  ...
  quiz: [
    {
      q: "WebSocketの常時接続をAPI Gatewayに任せると、自分たちが書くコードの責任範囲はどう変わるのでしょうか。",
      a: "接続を維持し続けるサーバーを持たなくてよくなり、自分のコードは接続時・メッセージ受信時・切断時のイベントごとに動くLambdaだけになります。数千接続の維持とスケーリングはAWS側の仕事になるため、常時起動サーバーの増減設計が不要です。一方で接続IDの台帳管理と宛先への配信ロジックは自作なので、「維持は任せる、配信は自作」という分担だと理解しておくと、代替案との比較を誤りません。"
    },
    {
      q: "接続IDをDynamoDBに保存しているのに、配信の失敗を受けて削除する掃除処理まで必要になるのはなぜでしょうか。",
      a: "切断の検知が完全ではなく、台帳に死んだ接続IDが残り続けるためです。通信断やアプリの強制終了では切断イベントが呼ばれないことがあり、放置すると配信のたびに無駄な送信と失敗が積み上がっていきます。配信時に410 Goneが返ったらその接続IDを消す、という後始末を入れておくのが定石です。じわじわ効いてくるタイプの問題なので、最初から組み込んでおきましょう。"
    },
    {
      q: "アプリ全体のAPIをGraphQLで統一する方針が決まったら、あなたならチャット部分をどう設計し直しますか。",
      a: "AppSync（代替1）へ寄せるのが自然です。サブスクリプション機能が接続管理・再接続・宛先への配信まで面倒を見てくれるので、推奨構成で自作していた接続IDテーブルと配信ループが丸ごと不要になり、リアルタイム部分の実装量が最小になるためです。判断材料はチャット単体の優劣ではなく、他の画面のデータ取得もGraphQLに寄せられるかどうかで、REST中心のチームでは学習コストが最大の障壁になります。"
    }
  ]
});
