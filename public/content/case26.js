...
registerCase({
  ...
  quiz: [
    {
      q: "RAGの回答品質が思わしくないとき、最初に手を付けるべきなのはどこでしょうか。",
      a: "多くの場合、生成モデルではなく検索です。関係のない文書チャンクを渡してしまえば、どれだけ賢いモデルでも正しくは答えられません。まずチャンクの分割サイズと、検索結果の上位何件をモデルへ渡すかを調整します。モデルの変更やプロンプトの作り込みに手を出すのはその後、というのが改善の順序です。"
    },
    {
      q: "この構成で「誰も使わなかった月」でも発生し続ける費用はどこでしょうか。",
      a: "ベクトル検索用のOpenSearchの常時稼働費で、月2万円から5万円と全体の大半を占めます。Bedrockは呼び出した分だけの従量課金なので、利用が少なければ費用も小さくなります。なお代替1のマネージドなナレッジベースに切り替えても裏側ではベクトルDBが動くため、<strong>マネージドにすれば固定費が消える</strong>わけではない点に注意が必要です。"
    },
    {
      q: "「まず総務部だけで3か月試したい。開発工数は最小に」と言われました。あなたならどの構成から始めるでしょうか。",
      a: "代替1のBedrockナレッジベースで始めます。チャンク分割・ベクトル化・同期・出典付与までマネージドで面倒を見てもらえ、アプリ側はAPIを1回呼ぶだけになるため、RAGに価値があるかの検証を最短で回せるからです。検索精度やプロンプトを追い込みたくなった部分だけ、後から自前構成へ移していくと失敗しにくくなります。"
    }
  ]
});
