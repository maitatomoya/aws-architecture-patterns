...
registerCase({
  ...
  quiz: [
    {
      q: "環境をIAMポリシーやタグで分けるのではなく、AWSアカウントそのものを分けるほうが強い分離だと言えるのはなぜでしょうか。",
      a: "アカウント境界は、明示的に許可しない限りデフォルトで遮断されるためです。IAMやタグによる分離は正しく書けていれば守られるという仕組みなので、ポリシーの書き漏れやタグの付け忘れが1つあれば破れてしまいます。アカウントを分ければ、開発の事故が本番へ波及する経路そのものが存在しなくなり、請求もサービスクォータも自然に分かれます。分離の強度はアカウント境界、IAM、タグ運用の順だと覚えておきましょう。"
    },
    {
      q: "CloudTrailの証跡を、ワークロードと同じアカウントではなく専用のログアカウントに集めるのはなぜでしょうか。",
      a: "本番アカウントが侵害されたときに、攻撃者が自分の痕跡を消せないようにするためです。同じアカウントにログを置くと、そのアカウントの権限を奪った相手はログの削除もできてしまい、証跡が最も必要な場面で失われます。ログを別アカウントのS3に集約し、他アカウントからは書き込めても消せない状態にしておけば、監査とインシデント調査の土台が守られます。バックアップを消せなくする発想（ケース48）と同じ考え方です。"
    },
    {
      q: "情シス2名の会社で「まずは請求と環境の分離だけできればよい、ガードレールは後回しでよい」と言われたら、あなたならどう提案しますか。",
      a: "一括請求だけの軽量マルチアカウント（代替2）から始めるのが現実的です。アカウントを分けるだけでも事故の影響範囲の分離という最大の効果は得られ、統制がなくても1アカウント同居よりはるかに安全になるためです。ただし最低限CloudTrailの有効化だけは全アカウント共通のルールにしておき、後からOrganizationsの全機能を有効化すればSCPと組織証跡のある推奨構成へ段階的に進めます。新規構築ならControl Towerで最初から整える選択肢も併せて示します。"
    }
  ]
});
