...
registerCase({
  ...
  quiz: [
    {
      q: "セッション情報をコンテナの中ではなくElastiCacheに置いているのはなぜでしょうか。これができていないとセール時に何が起きますか。",
      a: "コンテナ内にセッションがあると、負荷分散で別のコンテナへ振られた瞬間にカートの中身が消えたり、再ログインを求められたりします。ElastiCacheへ外出ししてあれば、どのコンテナに当たっても同じカートが見えるため、台数を自由に増減できます。スパイク対応のオートスケールは、状態を外に出す設計が前提になって初めて成立するのです。"
    },
    {
      q: "オートスケールを設定していても、セール前にECSタスク数の下限を手動で引き上げておくことがあります。なぜ自動任せにしないのでしょうか。",
      a: "オートスケールは負荷を検知してから新しいタスクを起動するため、立ち上がるまでに数十秒から数分のタイムラグがあります。セール開始直後のように数十倍のアクセスが一瞬で来る場合、その間に応答が詰まって機会損失になります。予測できるスパイクは事前に増やし、予測できない波は自動に任せる、という併用が実務の定石です。"
    },
    {
      q: "経営から「深夜はほぼ無人なのに固定費が高い」と指摘されました。あなたならどこから検討しますか。",
      a: "まずALB・NATゲートウェイ・Aurora・ElastiCacheという常時起動の内訳を出し、どれが本当に必要かを一つずつ確認します。波が極端でアイドル時間が長いならフルサーバーレス案が候補ですが、DynamoDBのデータ設計はRDBと発想が違うため移行コストが大きい点も同時に伝えます。現実的な落とし所は、在庫や決済など整合性が重要な部分をAuroraに残し、閲覧系だけを従量課金側へ寄せる併用です。"
    }
  ]
});
