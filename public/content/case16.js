...
registerCase({
  ...
  quiz: [
    {
      q: "EC2にcronを仕込む方式から乗り換える最大の理由は何でしょうか。単に費用の問題ではありません。",
      a: "cronを載せたサーバー自体が単一障害点になり、そこが落ちていると処理が実行されないまま誰にも気づかれないからです。EventBridge Schedulerはマネージドサービスなので、スケジューラが落ちていて動かなかったという失敗が起きません。加えてタイムゾーンを指定できるため、JSTで毎日二時と書けて時差によるバグも防げます。"
    },
    {
      q: "バッチの失敗通知を、バッチ自身のコードでメール送信するのではなく、CloudWatchアラームに任せるのはなぜでしょうか。",
      a: "バッチが異常終了したり、そもそも起動しなかったりした場合、そのバッチに書いた通知処理も当然動かないからです。監視は処理の外側に置いて初めて「動かなかったこと」を検知できます。この構図は実行環境がLambdaでもFargateでもBatchでも変わらない、共通のパターンです。"
    },
    {
      q: "月次集計だけが一時間近くかかることが分かりました。あなたなら全体をどう組み替えますか。",
      a: "スケジューラはEventBridge Schedulerのまま据え置き、その月次ジョブの実行先だけをECS Fargateのスケジュールタスクへ差し替えます。十五分の制限を超えたら実行環境を変えるという部分的な差し替えで済むのは、起点と実行環境を分けて設計しているからです。さらにジョブ同士の順序依存が出てきた段階で、AWS BatchのdependsOn指定を検討します。"
    }
  ]
});
