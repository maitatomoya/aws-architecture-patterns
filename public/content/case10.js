...
registerCase({
  ...
  quiz: [
    {
      q: "認証を自前で実装せずCognitoに任せるのは、単に楽だからではありません。どんな価値があるでしょうか。",
      a: "パスワードの保存方法、多要素認証、ソーシャルログイン連携はセキュリティ事故が起きやすい領域で、自作すると脆弱性の責任をすべて自分たちが負うことになります。Cognitoに寄せればその責任範囲を大きく減らせます。さらにトークン検証をAPI Gatewayのオーソライザーに置けば、各Lambdaから認証コードが消え、実装漏れによる認証すり抜けも防げます。"
    },
    {
      q: "「朝晩に集中し深夜はほぼゼロ」というモバイル特有のトラフィックが、なぜサーバーレス構成と相性が良いのでしょうか。",
      a: "API Gateway・Lambda・DynamoDBはいずれもリクエスト数と実行時間に対する課金なので、使われない時間帯の費用がほぼ発生しません。常時起動型ではアクセスがゼロの深夜も同じ料金がかかるため、稼働率が低いほど差が開きます。逆に二十四時間ほぼ一定の負荷があるサービスでは、この優位性は小さくなります。"
    },
    {
      q: "「RailsのAPIサーバーがすでに動いていて、これをモバイル向けに公開したい」と相談されました。あなたならサーバーレスへの書き換えを勧めますか。",
      a: "いいえ、まずはApp Runner+RDSへの載せ替えを勧めます。既存のコード・スキーマ・運用ノウハウをほぼ無修正で活かせて、移行リスクと工数が最小になるからです。認証だけ新規部分からCognitoへ寄せるなど、サーバーレス化は段階的に進めれば十分で、動いているものを書き直すのは最後の手段です。"
    }
  ]
});
