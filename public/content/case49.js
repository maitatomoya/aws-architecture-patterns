...
registerCase({
  ...
  quiz: [
    {
      q: "DR方式の比較を始める前に、まずRTOとRPOを決めるべきだとされるのはなぜでしょうか。",
      a: "この2つの数字が決まれば方式はほぼ自動的に決まるためです。バックアップ&リストア、パイロットライト、ウォームスタンバイ、マルチサイトの順にRTO/RPOが良くなり費用も上がる階段構造になっているので、目標値の上に方式を置く形になります。逆に方式から入ると、どこまでやれば十分かの基準がないまま費用だけの議論になります。RTO1時間・RPO数秒という本ケースの目標が、ウォームスタンバイを選ぶ根拠そのものです。"
    },
    {
      q: "「深夜でも人手なしでフェイルオーバーしたい」という要件は、この構成でどこまで満たせているでしょうか。",
      a: "自動で完了するのはRoute 53のヘルスチェックによるDNS切り替えまでです。Auroraグローバルデータベースのセカンダリ昇格は自動では発動しないため、夜間も対応できるオンコール体制を組むか、EventBridgeとLambdaで昇格の自動化を作り込む必要があります。RTO1時間を守れるかはこの部分の設計次第で、自動フェイルオーバーという言葉を鵜呑みにせず、どこまでが自動なのかを分解して確認するのが実務の勘所です。"
    },
    {
      q: "社内の勤怠システムについて「復旧は翌営業日でも構わない」と言われました。あなたならどの方式を選び、何を合意しますか。",
      a: "バックアップ&リストア方式（代替1）を選び、待機側は日次バックアップのコピーだけにして常時稼働するものをなくします。あわせて合意すべきはRPOで、日次バックアップなら最大24時間ぶんのデータが失われる可能性があることを数字で示して了解を得ます。復元手順をCloudFormation化しておかないと、いざというときテンプレートが動かないリスクも伝えます。全ワークロード一律ではなく、業務の重要度ごとに方式を混ぜるのが現実的な設計です。"
    }
  ]
});
