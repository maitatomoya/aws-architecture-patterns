...
registerCase({
  ...
  main: {
    ...
    points: [
      "Direct Connectは物理専用線なので、VPNと違って帯域・遅延が安定する。実は日次数十GB程度の転送量なら計算上はVPNの帯域でも運べるため、DXを選ぶ決め手は帯域の大きさそのものではなく、転送時間が毎日ぶれない安定性・低遅延・「インターネットを経由しない」ことを物理構成で説明できる監査要件にある",
      "Direct Connectは閉域（インターネット非経由）だが、回線そのものは通信を暗号化しない点に注意。監査で必ず聞かれる論点で、暗号化も必要なら回線レイヤーで暗号化するMACsec対応の接続を選ぶか、DXの上にVPNトンネルを重ねる（VPN over DX）方法がある",
      "ただし開通には物理工事と通信事業者との契約が必要で、数週間〜数か月かかる。プロジェクト計画に必ず織り込む",
      "S3をサブネットの中に描いていないのは作図の誤りではなくAWSの仕様。VPCの外にあるマネージドサービスへは、VPCエンドポイント経由で閉域のままアクセスする。なおゲートウェイ型エンドポイントはVPC内からの通信専用で、オンプレミスからは利用できない。オンプレから閉域のままS3へ届くには、インターフェイス型エンドポイント（PrivateLink）を使う",
  ...
  quiz: [
    {
      q: "日次数十GB程度なら計算上はVPNの帯域でも運べます。それでもDirect Connectを選ぶ決め手は何でしょうか。",
      a: "帯域の大きさそのものではなく、転送時間が毎日ぶれない安定性と低遅延、そして「インターネットを経由しない」ことを物理構成として説明できる点です。VPNはインターネット回線上のトンネルなので、経路が混雑すれば夜間バッチが翌朝までに終わらない日が出ます。止められない基幹連携ほど、平均値ではなく<strong>最悪値の予測可能性</strong>が効いてきます。"
    },
    {
      q: "「Direct Connectは閉域だから通信は保護されている」と説明したら、監査担当から指摘を受けました。どこが不正確だったのでしょうか。",
      a: "Direct Connectはインターネットを経由しない閉域ですが、回線そのものが通信を暗号化するわけではない点です。暗号化も要件なら、回線レイヤーで暗号化するMACsec対応の接続を選ぶか、Direct Connectの上にVPNトンネルを重ねる方法を取ります。閉域であることと暗号化されていることは別の性質だと整理しておくと、監査の場で答えに詰まりません。"
    },
    {
      q: "基幹連携が止まると業務も止まると分かりましたが、Direct Connectを2本引く予算はありません。あなたならどう設計するでしょうか。",
      a: "代替2のDirect Connectとサイト間VPNを組み合わせた冗長構成にします。予備回線のVPNは月5,500円程度で、2本目の専用線より大幅に安く物理障害や工事、事業者側の障害に備えられるからです。切替はBGPで自動化し、年1回はフェイルオーバー訓練で実際に切り替わることを確認します。切替後はVPNの細い帯域に縮退するため、優先して流す通信を事前に決めておく縮退運転の設計もセットで必要です。"
    }
  ]
});
