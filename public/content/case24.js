...
registerCase({
  ...
  quiz: [
    {
      q: "変換処理そのものはGlue、順序と失敗の管理はStep Functionsと役割を分けています。この分離には何の意味があるのでしょうか。",
      a: "リトライや分岐をアプリコードに埋め込むと、変換ロジックを直すたびに制御の仕組みまで壊すリスクを抱えます。Step Functionsに順序と失敗管理を宣言的に持たせれば、Glueジョブは変換に専念でき、「一時エラーは3回まで再試行、それでも失敗なら通知して停止」といった制御を設定変更だけで見直せます。実行履歴がフロー図として残り、どのステップで止まったか一目で分かるのも分離の効果です。"
    },
    {
      q: "S3を取り込み層と加工済み層に分けることが、夜間バッチの再実行のしやすさにどうつながるか説明してみましょう。",
      a: "途中で失敗しても取り込み層の元データが無傷で残るため、同じ入力から何度でもやり直せるからです。加工結果を元データに上書きしていると、1度失敗した時点で再実行の材料が失われます。あわせて、同じ入力なら何度実行しても同じ結果になる（冪等性）ようにジョブを書いておくことで、翌朝あわてて再実行しても安全という状態が作れます。"
    },
    {
      q: "処理対象が数百MBで変換も軽く、実行は必ず5分以内に終わると分かりました。あなたならどの構成を選ぶでしょうか。",
      a: "代替1のLambdaベースで十分です。Glueはジョブ起動に数十秒から数分のオーバーヘッドがあり、軽い処理には割高になるためです。ただしLambdaの実行時間上限は15分なので、データ増でそこに触れる見込みが少しでもあるなら、最初からGlueやBatchを選ぶほうが作り直しを防げます。ステップが増えてきたらLambda群の上にStep Functionsを被せる形でそのまま育てられます。"
    }
  ]
});
