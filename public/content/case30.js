...
registerCase({
  ...
  quiz: [
    {
      q: "Personalizeを使った推奨構成は「完全な従量課金にはならない」と説明しています。これはなぜでしょうか。",
      a: "リアルタイム推論のキャンペーンは最低スループット分を常時確保する仕組みで、推薦の呼び出しがゼロの時間帯もその分が課金されるからです。月2万円強がここで固定的にかかります。リアルタイム性を諦めて夜間に全ユーザー分の推薦を生成して保存しておくバッチ推薦に切り替えれば、この常時課金は大きく下げられます。"
    },
    {
      q: "代替2のルールベース推薦は「閲覧中の商品に応じて変わる」ものの、パーソナライズとは呼びません。この2つはどう違うのでしょうか。",
      a: "ルールベース推薦は「商品Aと同じ注文で買われた商品」を事前集計したものなので、同じ商品ページを見れば誰が見ても同じ結果が返ります。これは商品に紐づく推薦であって個人化ではありません。パーソナライズは利用者ごとの行動履歴で結果が変わることを指し、そこまでの体験が本当に必要かどうかがPersonalize導入の判断軸になります。"
    },
    {
      q: "立ち上げ期で行動ログがほとんど無く、推薦に効果があるかも分かっていません。あなたなら何から始めるでしょうか。",
      a: "代替2のルールベース推薦から始めます。購買履歴の集計だけで「よく一緒に買われる商品」は作れ、月数百円でクリック率が改善することも珍しくないからです。同時に行動ログの蓄積を始めておけば、それがそのままPersonalize移行時の学習データになります。効果測定のA/Bテストを先に設計しておくと、次の段階へ進む投資判断を数字で説明できます。"
    }
  ]
});
