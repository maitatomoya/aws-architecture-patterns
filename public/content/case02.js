...
registerCase({
  ...
  quiz: [
    {
      q: "推奨構成では、アップロードした画像をEC2のディスクではなくS3に保存しています。なぜわざわざ別のサービスへ逃がすのでしょうか。",
      a: "EC2に画像が溜まっていると、そのサーバーが壊れたときに画像も一緒に失われ、ディスク容量の管理も必要になります。状態をS3へ引き剥がしておけば、EC2は「いつ作り直してもよい使い捨ての箱」になり、障害時も新しいインスタンスを立て直すだけで復旧できます。DBをRDSへ、共有ファイルをEFSへ外出しするのもまったく同じ考え方です。"
    },
    {
      q: "CloudFrontを前段に置くと表示は速くなりますが、管理画面（/wp-admin）とログイン中のユーザーはキャッシュをバイパスさせる設定が必須です。忘れるとどんな事故が起きるでしょうか。",
      a: "誰か一人のログイン後のページがキャッシュされると、それが他のユーザーにも配信され、他人のログイン状態や未公開の下書きが見えてしまいます。記事を更新しても反映されない、ログインできないといった不具合の原因にもなります。キャッシュは「全員に同じものを返してよいコンテンツ」にだけ効かせる、という線引きを最初に決めておくことが重要です。"
    },
    {
      q: "運営から「コメント欄も検索機能も要らない。とにかくセキュリティ運用の手間を減らしたい」と言われました。あなたなら構成をどう変えますか。",
      a: "公開側に動的機能が不要なら、WordPressの静的書き出し案が有力です。WordPress本体を社内など公開しない場所で動かし、書き出したHTMLをS3+CloudFrontで配信すれば、攻撃対象となる公開サーバーそのものが消えます。ただし記事公開のたびに書き出しとキャッシュ削除の一手間が増えるため、その運用を編集部が受け入れられるかを先に確認します。"
    }
  ]
});
