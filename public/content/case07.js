...
registerCase({
  ...
  quiz: [
    {
      q: "MVPフェーズでは「一番月額が安い構成」が正解にならないことが多いのはなぜでしょうか。",
      a: "この段階で最も希少な資源はお金よりエンジニアの時間で、アイデアが当たるかを早く検証することが価値だからです。EC2一台構成は月額こそ安く見えますが、パッチ当て・監視・バックアップがすべて手作業になり、実際には人件費という形で高くつきます。App Runnerのようにインフラ設計を肩代わりする選択は、機能開発に使える時間を買う投資だと考えます。"
    },
    {
      q: "推奨構成の図にはインターネットゲートウェイがないのに、VPCは登場します。ここでVPCは何のために使われているのでしょうか。",
      a: "ユーザーからの入口はAWSが運用するApp Runnerの公開エンドポイントなので、自分のVPCをインターネットへ公開する必要がなく、入口としてのインターネットゲートウェイが登場しません。このVPCはDBを外から到達できない場所に隠すためだけに使われ、App RunnerからはVPCコネクタで内側へ入ります。VPCは必ずしもインターネット公開のための器ではない、という点を押さえておきましょう。"
    },
    {
      q: "チームにコンテナ経験者がおらず、Dockerfileを書くところで手が止まっていると相談されました。あなたならどう提案しますか。",
      a: "ZIPを渡すだけでEC2ベースの定番構成を組んでくれるElastic Beanstalk案へ切り替える判断が現実的です。裏側のEC2やALBが見えるためAWSの学習にもなり、後からロードバランサー付き構成やAuto Scalingへ設定だけで拡張できます。技術選定はサービスの優劣ではなくチームのスキルで決まることがある、という典型例です。"
    }
  ]
});
