...
registerCase({
  ...
  quiz: [
    {
      q: "3環境を揃えるとき、環境ごとにテンプレートを分けるのではなく「テンプレートは1つ、差分はパラメータだけ」という規律が本質だとされるのはなぜでしょうか。",
      a: "テンプレートを分けた瞬間に、手作業で環境を作っていた時代と同じ差異問題が再発するためです。片方だけに修正が入って気づかれないまま乖離し、「stgでは動くのにprodで動かない」に逆戻りします。同じテンプレートを配り、変えてよいのはインスタンスサイズや接続先などのパラメータだけと決めておけば、環境差はParameter Storeの値の差として一覧できる形に閉じ込められます。"
    },
    {
      q: "手動構築を続ける選択は「無料に見えて一番高い」と言われます。その理由を説明できるでしょうか。",
      a: "課金は発生しないものの、環境を再現できないという最大のコストを抱えるからです。障害復旧・環境複製・監査対応のすべてが担当者の記憶頼みになり、その人の異動や退職がそのまま事業リスクになります。環境差異による障害対応の人件費は継続的に発生し、事故1回の損失でIaCの学習コストを軽く上回ることが多いのが実態です。ツールの料金ではなく、再現できないことの代償で比較するのが正しい見方です。"
    },
    {
      q: "3人のチームで、いきなり環境ごとのアカウント分離まではやりきれないと言われました。あなたなら何から着手しますか。",
      a: "まず単一アカウントの中で、同じテンプレートからdev／stg／prodの3スタックを作るところから始めます（代替1）。命名規則とタグによる分離はIAMポリシーの書き漏れで破れる規約上の分離でしかない、と正直に共有したうえで、それでも手作業構築より大幅な前進だからです。テンプレートを1つに保っていれば、チームや監査要件が育ったときに展開先をアカウントへ変えるだけで推奨構成に移行できます。"
    }
  ]
});
