...
registerCase({
  ...
  quiz: [
    {
      q: "EventBridgeから各サービスのLambdaへ直接配信せず、受け手ごとにSQSを1枚挟んでいます。このキューが無いと何が困るのでしょうか。",
      a: "受け手が障害で停止している間のイベントが、リトライ上限を超えた時点で失われてしまいます。キューを挟めばイベントはそこに溜まり、復旧後に続きから処理できます。リトライ回数やデッドレターキューを受け手ごとに独立して設定できるのも、キューを1枚置く効果です。受け手の都合が送り手側のイベント損失にならない構造にするのが狙いです。"
    },
    {
      q: "イベントに「注文が確定した」ではなく「配送を手配せよ」と載せた場合、何が問題になるでしょうか。",
      a: "命令を載せた時点で、送り手が「誰が何をすべきか」を知っている状態に戻ってしまい、疎結合が崩れます。受け手が増えるたびに送り手のイベント設計を見直すことになり、APIを直接呼んでいた頃と実質的に変わりません。イベントには<strong>過去の事実</strong>だけを載せ、それを受けて何をするかは受け手の責任にするのが原則です。"
    },
    {
      q: "要件を見直したところ、振り分けは不要で「注文確定を全サービスへ同報するだけ」と分かりました。あなたなら構成をどう見直すでしょうか。",
      a: "代替1のSNS+SQSファンアウトへ寄せます。EventBridgeの内容ベースルーティングを使わないのであれば、配信が速くメッセージ単価も安いSNSで要件を満たせるからです。受け手ごとにSQSを挟む形はそのまま維持します。ただし後から「合計金額1万円以上の注文だけ別処理へ」のような本文の中身による振り分けが必要になったら、それがEventBridgeへ戻る分岐点になります。"
    }
  ]
});
