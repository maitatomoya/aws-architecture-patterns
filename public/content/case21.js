...
registerCase({
  ...
  quiz: [
    {
      q: "受け口にKinesis Data Streamsを置き、その先にFirehoseを繋いでいます。Firehoseだけで受けてはいけないのでしょうか。",
      a: "Firehoseは受けたデータを配達先へ届けることに特化しており、同じデータを別の用途で読み直すことができません。Data Streamsはイベントを一定期間保持し、複数の読み手が独立に読めるため、S3への蓄積と即時集計を同じストリームから並走させられます。将来の用途追加に備えて受け口をStreamsにしておく、という判断がこの構成に埋め込まれています。"
    },
    {
      q: "この構成でイベント量が10倍になったとき、費用はどこから増え始めるでしょうか。",
      a: "まずKinesisのシャード数です。1シャードは秒間1MBかつ1,000件が上限なので、流量に比例してシャードを増やすことになり、その時間課金が積み上がります。次に効いてくるのがAthenaのスキャン量で、Parquet化と日付パーティションを怠っていると分析のたびの課金が伸びます。S3の保存料は相対的に小さく、蓄積量よりも<strong>流量と読み方</strong>が費用を決めます。"
    },
    {
      q: "「キャンペーンの現在申込数を管理画面に秒単位の鮮度で表示したい」と言われました。あなたなら構成をどう足すでしょうか。",
      a: "推奨構成のAthena経由では応答に数秒から数十秒かかるため要件を満たせません。代替1のLambda+DynamoDBを同じストリームから並走させ、集計値を事前計算してDynamoDBへ書き、画面はキー指定の読み取りだけで取得する形にします。クエリを速くするのではなく<strong>クエリを不要にする</strong>発想です。S3への蓄積をFirehoseで維持したまま追加できるのが、受け口をStreamsにしておいた利点です。"
    }
  ]
});
