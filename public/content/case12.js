...
registerCase({
  ...
  quiz: [
    {
      q: "ダブルブッキングの防止を、アプリで「空きを確認してから登録する」という手順で実装してはいけないのはなぜでしょうか。",
      a: "確認してから登録するまでのわずかな時間に別の人が同じ枠を取ると、両方とも空いていたと判断して二重予約が成立してしまうからです。DynamoDBの条件付き書き込みを使えば、まだ空いている場合のみ書き込むという判定と書き込みがデータベース側で一体になり、同時アクセスでも矛盾が起きません。整合性はアプリのif文ではなくDBの仕組みで守る、という原則です。"
    },
    {
      q: "リマインドを「毎分バッチを動かして送信対象を探す」方式ではなく、EventBridge Schedulerのワンタイムスケジュールで実現しているのはなぜでしょうか。",
      a: "毎分のポーリングは予約が一件もない深夜でも動き続け、無駄な実行と課金が発生します。予約一件ごとに送信時刻のスケジュールを作れば、必要なときだけ一回実行され、実行後の自動削除も設定できます。処理を「探しに行く」から「時が来たら呼ばれる」に変えるのが、イベント駆動の発想です。"
    },
    {
      q: "「複数店舗をまたいで、担当スタッフとメニューの条件で空き枠を横断検索したい」という要望が出ました。あなたならどうしますか。",
      a: "条件の組み合わせで絞り込む検索はリレーショナルDBの得意分野なので、RDS案への切り替えを検討します。DynamoDBでも設計次第で対応できますが、キー設計の工夫が増えて開発速度が落ちがちです。RDSにする場合は多数のLambdaからの同時接続で接続数が枯渇しやすいため、RDS Proxyを挟む前提と、常時起動ぶんの固定費が増えることもあわせて説明します。"
    }
  ]
});
