...
registerCase({
  ...
  quiz: [
    {
      q: "この構成でカスタム用語集の整備が実務上の要になるのはなぜでしょうか。",
      a: "機械翻訳は文脈から訳語を選ぶため、施設名や地名などの固有名詞が案内ごとに違う訳になったり、意訳されて意味が変わったりするからです。対訳表をTranslateに登録して訳語を固定すれば、6言語すべてで表記が揃います。用語集を育てていく作業そのものが、この構成における翻訳品質の運用だと考えておくとよいでしょう。"
    },
    {
      q: "この構成の骨格は、帳票OCR（ケース28）や文字起こし（ケース29）とほとんど同じ形をしています。共通しているのはどんな型でしょうか。",
      a: "「S3にファイルが置かれたイベントでLambdaが起動し、AIサービスを呼んで、結果をDynamoDBなどへ保存する」という型です。真ん中のサービスがTextract、Transcribe、Translateと入れ替わるだけで、VPCもサーバーも登場せず、処理が必要なときだけ動きます。この型を1つ理解しておけば、AIサービスを差し替えるだけで別の業務を自動化できるのがサーバーレスとAIの組み合わせの強みです。"
    },
    {
      q: "「運行案内は今のままでよいが、観光ガイドの読み物が直訳調で使えない」と指摘されました。あなたなら構成をどう変えるでしょうか。",
      a: "文書の種別でパイプラインを分けます。定型の運行案内は速くて安いTranslateのまま残し、読み物のガイド文だけ代替1のBedrockへ回して、トーンの統一や文化的な補足まで指示文で指定します。全文書をLLMに通すとコストも処理時間も増えるため、品質が要る文書に絞るのが費用管理の鍵です。LLMは訳抜けが起こりうるので、文字数比のチェックなど検証も挟みます。"
    }
  ]
});
