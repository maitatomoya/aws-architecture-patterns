...
registerCase({
  ...
  quiz: [
    {
      q: "対戦中の通信をAPI Gateway経由にせず、割り当てられたゲームサーバーへ直接接続させているのはなぜでしょうか。",
      a: "対戦中は一秒間に何度も座標や操作を送り合うため、HTTPのAPIを経由すると往復のオーバーヘッドが遅延として積み上がり、操作感が破綻します。マッチング前後の処理だけをAPIに任せ、対戦中はUDPで直接つなぐという役割分担が低遅延の要です。何を経由させないかを決めるのが、リアルタイム系の設計の中心になります。"
    },
    {
      q: "永続データはDynamoDB、ランキングはElastiCacheと使い分けているのはなぜでしょうか。",
      a: "戦績やアイテムは失われては困るデータなので、耐久性のある永続ストアが適します。一方ランキングは書き込みが激しく常にスコア順で読むため、メモリ上で高速に並べ替えられるElastiCacheが向いています。消えて困るデータか、速さが命のデータかでストアを選び分けるのは、ゲームに限らず高トラフィックのシステム全般で使える定石です。"
    },
    {
      q: "開発中のゲームが将棋のようなターン制に変わり、ミリ秒単位の低遅延は不要になりました。あなたなら構成をどう変えますか。",
      a: "GameLift Serversをやめ、API GatewayのWebSocket APIとLambda、DynamoDBに寄せる判断が有力です。イベントがたまに飛ぶ通信ならこれで十分で、プレイヤーがいない時間帯のコストがほぼゼロになり、ゲームサーバーの実装と運用そのものが不要になります。ただしLambdaに状態は残せないため、盤面や手番はすべてDynamoDBへ置く設計に切り替えます。"
    }
  ]
});
