...
registerCase({
  ...
  quiz: [
    {
      q: "変換後の画像を元画像と同じバケット（同じプレフィックス）へ書き戻してはいけないのはなぜでしょうか。",
      a: "書き込みが再びS3イベントを発火させ、そのイベントで起動したLambdaがまた画像を書き込む、という無限ループになるからです。実行料金が青天井に膨らむ、現場で実際に起きる事故です。入力と出力の置き場所を分けるという単純なルールで構造的に防げるので、最初から徹底しておきましょう。"
    },
    {
      q: "アップロードを自前のAPIサーバー経由ではなく、署名付きURLでS3へ直接行わせているのはなぜでしょうか。",
      a: "数MBの画像がAPIサーバーを通らないため、アップロードの帯域と処理負荷がアプリ側にまったくかからないからです。署名付きURLは有効期限つきで発行できるので、誰でも書き込めるバケットにする必要もありません。大きなファイルの受け渡しはストレージへ直接させる、というのは他のパターンでも応用できる定石です。"
    },
    {
      q: "「リサイズの前に不適切画像の判定を入れ、そのあと透かしも入れたい」と言われました。あなたなら構成をどう変えますか。",
      a: "工程が複数になり、それぞれで失敗リトライや進捗の把握が必要になるため、Step Functionsで多段のワークフローへ組み替えるのが有力です。一つのLambdaに詰め込むとエラー処理でコードが肥大化し、どの工程で失敗したのかも追いにくくなります。なおS3のイベント通知はStep Functionsを直接起動できないため、S3のEventBridge統合を有効にして経由させる点も押さえておきます。"
    }
  ]
});
