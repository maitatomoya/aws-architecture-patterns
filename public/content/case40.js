...
registerCase({
  ...
  quiz: [
    {
      q: "地図を外部の地図サービスではなくLocation Serviceで用意すると、設計上どんな心配ごとが減るでしょうか。",
      a: "地図の利用契約・APIキーの管理・利用規約の確認といった、技術以外の管理業務をAWSに寄せられます。さらに位置情報という機微なデータがAWSの外へ出ないため、コンプライアンス面の説明もしやすくなります。引き換えに地図の見た目や機能のカスタマイズ性は専業サービスより限定的なので、独自の地図表現が要件にあるかどうかが分かれ目になります。"
    },
    {
      q: "位置を1件ずつ送らず、最大10件をまとめたバッチ送信にしているのはなぜでしょうか。",
      a: "位置の書き込みがAPI呼び出し単位の従量課金だからです。10件まとめて送れば呼び出し回数が最大で10分の1になり、車両が増えても費用の伸びを抑えられます。あわせてトラッカーの位置フィルタリングを使えばGPS誤差による細かいジグザグを間引けるので、費用だけでなく地図表示の見た目も改善します。従量課金のサービスでは、何が1回として数えられるのかを意識するのが設計の基本です。"
    },
    {
      q: "車両が3,000台に増え、更新間隔も数秒になり、急ブレーキ検知もしたいと言われたら、あなたならどうしますか。",
      a: "位置の受け口を代替1のKinesis Data Streamsへ切り替え、Lambdaでリアルタイム判定、Firehoseで全履歴をS3へ、と役割を分けます。1本のストリームに複数の読み手を付けられるので、即時判定と蓄積分析を同じ入口で両立できるからです。ここで重要なのは、地図表示とジオフェンスはKinesisに含まれないため、Location Serviceを地図・通知係として併用する点で、この2案は排他ではありません。"
    }
  ]
});
