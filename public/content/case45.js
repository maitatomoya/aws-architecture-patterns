...
registerCase({
  ...
  quiz: [
    {
      q: "「ECSかEKSか」の前に決めるべきことがある、とこのケースは述べています。それは何で、なぜ先に決めるのでしょうか。",
      a: "ホスト（コンテナが動くサーバーのOS）を自分で管理するかどうかです。Fargateを選べばOSのパッチ当てやホスト監視という定常業務が丸ごと消えるので、専任のインフラ担当がいないチームでも運用が成立します。オーケストレーターの比較は、その次に来る「どこまで細かく制御したいか」の話です。順序を逆にすると、運用体制に合わない基盤を選んでしまいます。"
    },
    {
      q: "Kubernetes未経験の4人チームが「将来のためにEKS」を選ぶのは、なぜ過剰投資になりやすいのでしょうか。",
      a: "EKSの価値である業界標準APIとエコシステムは、複数チームで多数のサービスを運用して初めて効いてくる一方、クラスターのバージョンアップやアドオン管理といった継続的な運用作業は最初から発生するためです。コントロールプレーンだけで月約73USDの固定費もかかります。サービスが1〜数個ならECSで十分で、チームとサービス数が育ってから移行しても遅くありません。イメージはECRに置いたままなので、乗り換え自体は可能です。"
    },
    {
      q: "アクセスの少ない社内向けWebアプリを1つ、最小の手間で公開したいと頼まれたら、あなたならどれを選びますか。",
      a: "App Runner（代替2）が第一候補です。VPC・ALB・証明書・スケーリングの設計をApp Runnerが内部で肩代わりするため公開までの手数が最小で、アイドル時はCPU課金が止まるのでアクセスの少ないサービスではECSの常時起動より安くなることが多いからです。RDSなどVPC内のリソースへもVPCコネクタで接続できます。長時間のWebSocket接続や細かいネットワーク制御が必要になったらECSへ移せばよく、イメージはそのまま使えます。"
    }
  ]
});
